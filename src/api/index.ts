...
const PORT = Number.parseInt(process.env.MIKROMEET_PORT || process.env.PORT || "3000", 10);
...
function parseOptionalParticipantLimit(value: unknown): number | undefined {
  if (value === undefined || value === null || value === "") return undefined;
    ...
    if (url.pathname === "/") {
      ...
      req.on("end", () => {
        try {
          ...
          const scheduledStart = parseOptionalTimestamp(options.scheduledStart);
          const scheduledEnd = parseOptionalTimestamp(options.scheduledEnd);
          ...
          const roomId = parseOptionalString(options.roomId, 12)?.toUpperCase();
          ...
          const result = roomManager.preCreateRoom({
            roomId,
            password: parseOptionalString(options.password, 50),
            maxParticipants: parseOptionalParticipantLimit(options.maxParticipants),
            title: parseOptionalString(options.title, 80),
            scheduledStart,
            scheduledEnd,
          });
...
server.listen(PORT, () => {
  ...
  console.log(`MikroMeet server running on ${protocol}://127.0.0.1:${PORT}`);
  console.log(`Health check: ${protocol}://127.0.0.1:${PORT}/health`);
  console.log(`Config endpoint: ${protocol}://127.0.0.1:${PORT}/config`);
});
