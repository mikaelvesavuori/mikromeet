...
export class SignalingServer {
    ...
    try {
      ...
      switch (message.type) {
        ...
        case "recording-started":
          this.handleRecordingStarted(socket);
          break;
        case "recording-stopped":
          this.handleRecordingStopped(socket);
          break;
        case "reaction":
          this.handleReaction(socket, message as ReactionMessage);
          break;
  ...
  private requireParticipant(socket: WebSocketLike): Participant | null {
    const participant = this.participants.get(socket);
    if (!participant) {
      this.sendError(socket, "Not joined to a room", "NOT_JOINED");
      return null;
    }
    return participant;
  }

  private requireModerator(socket: WebSocketLike, unauthorizedMessage: string): Participant | null {
    const participant = this.requireParticipant(socket);
    if (!participant) return null;

    if (!participant.isModerator) {
      this.sendError(socket, unauthorizedMessage, "UNAUTHORIZED");
      return null;
    }
  ...
  private handleRecordingStarted(socket: WebSocketLike): void {
    const participant = this.requireModerator(socket, "Only moderators can start recording");
    if (!participant) return;
  ...
  private handleRecordingStopped(socket: WebSocketLike): void {
    const participant = this.requireModerator(socket, "Only moderators can stop recording");
    if (!participant) return;
  ...
  private handleReaction(socket: WebSocketLike, message: ReactionMessage): void {
    const participant = this.requireParticipant(socket);
    if (!participant) return;

    const reaction = message.reaction.trim().slice(0, 8);
    if (!reaction) {
      this.sendError(socket, "Reaction cannot be empty", "INVALID_REACTION");
      return;
    }
    ...
    this.roomManager.broadcast(participant.roomId, reactionMessage, participant.id);
  }

  private handleModeratorAction(socket: WebSocketLike, message: ModeratorActionMessage): void {
    const participant = this.requireModerator(socket, "Only moderators can perform this action");
    if (!participant) return;
    ...
    if (!targetParticipant) {
      this.sendError(socket, "Target participant not found", "TARGET_NOT_FOUND");
      return;
    }
    ...
    const updateMessage: ParticipantUpdatedMessage = {
      ...
      isModerator: targetParticipant.isModerator,
  ...
  private handleRoomLock(socket: WebSocketLike): void {
    const participant = this.requireModerator(socket, "Only moderators can lock the room");
    if (!participant) return;
  ...
  private handleRoomUnlock(socket: WebSocketLike): void {
    const participant = this.requireModerator(socket, "Only moderators can unlock the room");
    if (!participant) return;
  ...
  private handleAdmitUser(socket: WebSocketLike, message: AdmitUserMessage): void {
    const participant = this.requireModerator(socket, "Only moderators can admit users");
    if (!participant) return;
  ...
  private handleRejectUser(socket: WebSocketLike, message: RejectUserMessage): void {
    const participant = this.requireModerator(socket, "Only moderators can reject users");
    if (!participant) return;
