...
async function fetchRuntimeConfig(): Promise<Response> {
  const preferredResponse = await fetch("/config.json", { cache: "no-store" });
  if (preferredResponse.ok) {
    return preferredResponse;
  }

  const legacyResponse = await fetch("/mikromeet.config.json", { cache: "no-store" });
  if (!legacyResponse.ok) {
    throw new Error(`Failed to load config: ${legacyResponse.status}`);
  }
