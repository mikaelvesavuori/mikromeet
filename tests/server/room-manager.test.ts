...
describe("RoomManager", () => {
  ...
  const persistencePath = "/tmp/mikromeet-room-manager-test-rooms.json";
  ...
  describe("preCreateRoom", () => {
    it("stores scheduling metadata", () => {
      const startsAt = Date.UTC(2026, 3, 30, 12, 0, 0);
      const endsAt = Date.UTC(2026, 3, 30, 12, 30, 0);
      ...
      expect(roomManager.getRoom("ABC123")).toMatchObject({
        title: "Roadmap sync",
        scheduledStart: startsAt,
        scheduledEnd: endsAt,
      });
