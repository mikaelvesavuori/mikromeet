...
describe("RoomManager Integration Tests", () => {
  ...
  const persistencePath = "/tmp/mikromeet-integration-test-rooms.json";

  beforeEach(() => {
    rmSync(persistencePath, { force: true });
    roomManager = new RoomManager({ persistencePath });
  });
...
describe("SignalingServer Integration Tests", () => {
  ...
  const persistencePath = "/tmp/mikromeet-signaling-integration-test-rooms.json";
  ...
  describe("Reactions", () => {
    it("broadcasts reactions to other participants", () => {
      const socket1 = new TestWebSocket();
      const socket2 = new TestWebSocket();
      ...
      const reaction = socket2.sentMessages
  ...
  describe("Moderator Actions", () => {
    ...
    it("should not allow non-moderator to kick", () => {
      ...
      expect(JSON.parse(errorMsg ?? "{}").code).toBe("UNAUTHORIZED");
    });

    it("should broadcast recording state from moderators", () => {
      const modSocket = new TestWebSocket();
      const userSocket = new TestWebSocket();
      ...
      modSocket.sentMessages = [];
      userSocket.sentMessages = [];
      ...
      const recordingMessage = userSocket.sentMessages
        .map((message) => JSON.parse(message))
        .find((message) => message.type === "recording-started");

      expect(recordingMessage).toBeDefined();
      expect(recordingMessage.startedBy).toBeTruthy();
    });
  });
