...
describe("SignalingServer", () => {
  ...
  beforeEach(() => {
    roomManager = new RoomManager({ persistencePath: "/tmp/mikromeet-test-rooms.json" });
  ...
  describe("join handling", () => {
    ...
    it("should reject join when room is full", () => {
      ...
      expect(errorMessage.code).toBe("ROOM_FULL");
    });
  ...
  describe("relay messages", () => {
    ...
    it("should return error for unknown message type", () => {
      ...
      expect(errorMessage.code).toBe("UNKNOWN_MESSAGE_TYPE");
    });
    ...
    it("should handle invalid JSON gracefully", () => {
      ...
      expect(errorMessage.code).toBe("INVALID_MESSAGE");
    });
  });
});
