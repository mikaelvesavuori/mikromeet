...
describe("SignalingClient", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    client = new SignalingClient("ws://127.0.0.1:3000/ws");
  });
  ...
  describe("send methods", () => {
    ...
    it("should send reaction message", () => {
      const mockWs = getMockWs();
      client.sendReaction("room-123", "user-1", "thumb");

      expect(mockWs.send).toHaveBeenCalledWith(expect.stringContaining('"type":"reaction"'));
      expect(mockWs.send).toHaveBeenCalledWith(expect.stringContaining('"reaction":"thumb"'));
    });
  });
